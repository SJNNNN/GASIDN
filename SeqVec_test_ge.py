...
def train(model, epoch):
    ...
    val_features, val_graphs, val_labels ,val_paddingfeatures= load_data(Dataset_Path + "test_new.txt",
    ...
    for epoch in range(epoch):
        ...
        if best_acc < valid_acc:
            ...
            best_epoch = epoch + 1
            cur_epoch = 0
            torch.save(model.state_dict(), os.path.join('./model/ge_SeqVec_best_model_onelayer.pkl'))
        else:
            cur_epoch += 1
            if (cur_epoch > 200):
                break
...
def main():
    ...
    f = open("./result/ge_independ_Roc_result_onelayer.txt", 'w', encoding="utf-8")
